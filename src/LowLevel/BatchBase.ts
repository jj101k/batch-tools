 ...
 * by the caller.
 */
export abstract class BatchBase<I extends IX, O extends OX, IX = I, OX = O> extends ExtensiblePromise<OX[]> implements Batchable<I, OX> {
    /**
     *
     */
     ...
     */
    protected promise: Promise<OX[]>

    /**
     *
     */
    protected selectionBuffer: SelectionBuffer<I, IX>
     ...
     * @returns
     */
    protected async getResults(worker: (...ts: I[]) => Promise<O[]>, backlog: IX[]): Promise<OX[]> {
        this.storedInternalState = BatchState.Sent
        this.debugLog("Selection resolved with buffer", backlog)
        const results = await worker(...this.workableBacklog(backlog))
        if(this.internalState == BatchState.Aborted) {
     ...
     * @returns
     */
    protected abstract workableBacklog(backlog: IX[]): I[]
     ...
     * @param delay
     */
    protected postInit(worker: (...ts: I[]) => Promise<O[]>,
        sendCondition?: BatchSendCondition<I>, delay = false): void {
        // No-op
    }
    ...
    constructor(worker: (...ts: I[]) => Promise<O[]>,
        ...
        this.postInit(worker, sendCondition, delay)

        this.promise = this.selectionBuffer.then(backlog => this.getResults(worker, backlog)).finally(() => {
